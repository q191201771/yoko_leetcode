...
std::string stringify_stl(std::stack<T> val,
  ...
  ss << style.delimiter_whole_prefix_;
  for (; ; ) {
    ss << style.delimiter_item_prefix_ << stringify_stl_one_(val.top()) << style.delimiter_item_suffix_;
    val.pop();
    if (val.empty()) { break; }
    else { ss << style.delimiter_item_sep_; }
...
std::string stringify_stl(std::queue<T> val,
  ...
  ss << style.delimiter_whole_prefix_;
  for (; ; ) {
    ss << style.delimiter_item_prefix_ << stringify_stl_one_(val.front()) << style.delimiter_item_suffix_;
    val.pop();
    if (val.empty()) { break; }
    else { ss << style.delimiter_item_sep_; }
